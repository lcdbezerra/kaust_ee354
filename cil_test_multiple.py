...
def main():
	...
	try:
		...
		transform_list = random.sample(world.get_map().get_spawn_points(), nVehicles)
		...
		for i in range(nVehicles):
			...
			camera_list[-1].listen(queue_list[-1].put)
		...
		while True:
			...
			# 		break
			for i in range(nVehicles):
				image = queue_list[i].get()
				im = np.array(image.raw_data).reshape(imageSize)[:,:,:3]
				speed_vec = vehicle_list[i].get_velocity()
				...
				vehicle_list[i].apply_control(control)
