...
########### USE THIS TO PREVENT GPU FROM BEING USED ###############
...
class ImitationLearningAgent(Agent):
    ...
    def _compute_action(self, rgb_image, speed, direction=None):
        ...
        im = im.resize((self._image_size[1], self._image_size[0]))
        image_input = np.array(im)
        ...
        ########################################
        # print("STEER, type: %s,\tshape: %s,\tvalue: %s,\tdir: %s" % (type(steer), steer.shape, steer, dir(steer)))
